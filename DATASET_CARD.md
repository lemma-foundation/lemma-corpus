# Dataset Card

## Dataset

Lemma Corpus SN467 smoke artifacts.

## Contents

The checked-in artifact set contains 4 accepted Lean proof rows, the pinned registry used to select the active K-window, and compact JSONL exports for downstream inspection.

## Verification

The validator accepted all 4 proofs with the pinned Lean verifier. Schema validation passed before export.

## Privacy Boundary

This repository is public-data only. Do not publish validator receipts, submission spools, wallet files, local paths, logs, environment files, private notes, or raw failed submissions.

## License

Rows carry source-level license metadata. The smoke rows are generated test data marked `CC-BY-4.0` in the registry.
